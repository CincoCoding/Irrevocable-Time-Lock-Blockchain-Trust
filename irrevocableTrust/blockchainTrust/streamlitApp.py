...
with st.sidebar:
    ...
    _beneficiary1_key = st.text_input("Beneficiary One Private Key")

    _beneficiary2_key = st.text_input("Beneficiary Two Private Key")
...
with st.sidebar:
    ...
    b = st.button('Sign and deploy Contract')
    if b:
...
# Transact payable solidity function receiveMoney that moves ETH from wallet to contract
    st.session_state.receiveMoneyTransaction = st.session_state.timelock_contract_instance.functions.deposit().buildTransaction(
    {
    "gasPrice" : w3.eth.gas_price,
    'from': trusteeAddress,
...
f = st.button("sign transaction to deposit")
if f:
    st.session_state.receiveMoneysigned_transaction = w3.eth.account.sign_transaction(st.session_state.receiveMoneyTransaction, private_key=str(privateKey))
    st.write("#### Receive Money Transaction Signed!")

    # Send transaction
    st.session_state.receiveMoneytransaction_hash = w3.eth.send_raw_transaction(st.session_state.receiveMoneysigned_transaction.rawTransaction)
    st.write("#### Receive Money Transaction Sent!")

    # Get receipt
    st.session_state.receiveMoneytransaction_receipt = w3.eth.waitForTransactionReceipt(st.session_state.receiveMoneytransaction_hash)
    st.write("#### Receive Money Receipt")
    st.write(st.session_state.receiveMoneytransaction_receipt)

    # Display contract address
    st.session_state.contract_address = str(st.session_state.timelock_contract_instance.address)
    st.write("## Contract Address: ")
    st.write(f"### {st.session_state.contract_address}")

    # Display contract balance
    st.session_state.contract_balance = w3.eth.getBalance(st.session_state.contract_address)
    st.write("## Contract Balance: ")
    st.write(f"### {st.session_state.contract_balance}")   


# withdraw solidity function that moves ETH from contract back to wallet after duration

#New nonce counter for the beneficiary wallet performing the transactions
st.session_state.wallet_nonce = w3.eth.getTransactionCount(beneficiary1Address)

# Button to withdraw the specified amount from the smart contract, and deposit it into the beneficiary addresses
g = st.button('Build Withdraw from contract transaction')
if g:

    #builds the withdraw transaction. This transaction is created and sent from the beneficiary account, so we need to reset the nonce to 0, add that address as the "from" sender, and add it's private key below.
    st.session_state.withdrawal = st.session_state.timelock_contract_instance.functions.withdraw().buildTransaction(
 {
    "gasPrice" : w3.eth.gas_price,
    #"chainID" : 1337,
    ...
    st.session_state.wallet_nonce += 1
    st.write(st.session_state.withdrawal)
    
#Signs the transaction using the beneficiaries private key

    st.session_state.withdrawal_transaction = w3.eth.account.sign_transaction(st.session_state.withdrawal, private_key=(_beneficiary1_key))
    st.write("#### Withdraw Transaction Signed!")

    #Sends the withdraw transaction
    st.session_state.withdrawal_hash = w3.eth.send_raw_transaction(st.session_state.withdrawal_transaction.rawTransaction)
    st.write("#### Receive Money Transaction Sent!")

    #displays the receipt for the withdraw transaction
    st.session_state.withdrawal_receipt = w3.eth.waitForTransactionReceipt(st.session_state.withdrawal_hash)
    st.write("#### Withdrawal Receipt")
    st.write(st.session_state.withdrawal_receipt)
