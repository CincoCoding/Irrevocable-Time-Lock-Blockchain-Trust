...
_trustee = st.sidebar.selectbox("Select Trustee", options=accounts)

# # Use a Streamlit component to get the address of the beneficiary one from the user
_beneficiary1 = st.sidebar.selectbox("Select Beneficiary One", options=accounts)

# # Use a Streamlit component to get the address of the beneficiary two from the user
_beneficiary2 = st.sidebar.selectbox("Select Beneficiary Two", options=accounts)

# Display trustee balance
trustee_balance = str(w3.eth.getBalance(_trustee))
st.write("## Trustee Balance: ")
st.write(f"### {trustee_balance}")
         
# # Display beneficiary1 balance
# beneficiary1_balance = str(w3.eth.getBalance(_beneficiary1))
# st.write("## Beneficiary One Balance: ")
# st.write(f"### {beneficiary1_balance}")

# # Display beneficiary2 balance
# beneficiary2_balance = str(w3.eth.getBalance(_beneficiary2))
# st.write("## Beneficiary Two Balance: ")
# st.write(f"### {beneficiary2_balance}")
...
waitingPeriod = st.text_input("Waiting Period")

paymentPerPayoutPeriod = st.text_input("Payment")

st.write("Initial payout interval")
#waitingPeriod = 1
st.write("Time Interval")
#paymentPerPayoutPeriod = 1000

#_beneficiary1 = st.text_input("Initial Waiting Period For First Withdrawal")

#_beneficiary2 = st.text_input("Waiting Period Between Withdrawals")

#trying to cast variables to proper type
#_trustee_ABI = w3.toChecksumAddress(_trustee)

#_beneficiary1_ABI = w3.toChecksumAddress(_beneficiary1)

#_beneficiary2_ABI = w3.toChecksumAddress(_beneficiary2)
...
#     #Transact payable solidity function deposit that moves ETH from wallet to contract
...
#     st.write(f"### {contract_balance}")   
    ...
    wallet_nonce = 0 
if st.button("Withdraw From Contract"):
    ...
    withdrawal_transaction = w3.eth.account.sign_transaction(withdrawal, private_key=str(_beneficiary1_key, _beneficiary2_key))
    ...
    withdrawal_receipt = w3.eth.waitForTransactionReceipt(withdrawal_hash)
    st.write("#### Withdrawal Receipt")
    st.write(withdrawal_receipt)