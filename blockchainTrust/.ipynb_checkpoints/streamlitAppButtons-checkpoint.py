...
def load_contract():
    ...
    #  with open(Path('./contracts/compiled/timeLock_bytecode.json')) as r:
    #     timeLock_bytecode = json.load(f)
...
private_key = "90e541c69b16e34e5b7bcfea6e77e1bd355d495e3e8f5610aab58db6864c7bd4"


nonce = w3.eth.getTransactionCount(trustee_address)


global transaction, signed_transaction, transaction_hash, transaction_receipt

if st.button("deploy contract"):
    transaction = timeLock_contract.constructor(trustee_address).buildTransaction(dict({"gasPrice": w3.eth.gas_price,"chainId": 1337, "from": trustee_address,"nonce": nonce}))
...
if st.button("Sign Transaction"):
    ...
    st.write("#### Transaction Signed")

if st.button("Send Transaction"):
    transaction_hash = w3.eth.send_raw_transaction(signed_transaction.rawTransaction)
    st.write("#### Transaction Sent!")
    
if st.button("Transaction Receipt:"):
